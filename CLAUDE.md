...
The documentation is organized into **6 goal-first tabs** (hybrid axis: goals at the top, products nested one level deeper, persona on-ramps on the homepage):

1. **Get Started** — "What is SafeDep?" primer, persona routing, quickstarts, core concepts
2. **Protect Developers** — install-time and AI-coding protection (pmg, MCP, Endpoint Sync)
3. **Scan & Analyze** — repository scanning (vet), SBOM (xBom), AI governance, CI/CD integrations
4. **Govern & Manage** — SafeDep Cloud operations: visibility, policy, access (Endpoint Hub, etc.)
...
Every placement decision derives from these six rules. Full rationale: `docs-ia-proposal.md`.
...
1. **Tutorials** (`introduction.mdx`, `*/quickstart.mdx`) — Learning-oriented, hands-on lessons for beginners. Live in Get Started or at the top of a goal tab.
2. **How-to Guides** (organized by goal, not tool) — Live in the relevant goal tab (Protect Developers / Scan & Analyze / Govern & Manage):
...
3. **Reference** (`vet/advanced/`, `api-reference/`) — Technical specifications and lookup resources. All live in the **Reference** tab.
4. **Explanation / Concepts** (`concepts/`, `*/concepts/`) — Understanding-oriented conceptual content. Live in Get Started › Core Concepts. *Note: concept pages are first-class, atomic, and linkable (R4).*

**Diátaxis → location quick map:** Tutorial → Get Started · How-to → relevant goal tab · Reference → Reference tab · Explanation → Get Started › Core Concepts.
...
│
├── concepts/              # ★ Goal-based: atomic concept pages (new content goes here)
├── get-started/           # ★ Goal-based: persona routing, quickstart aggregators
├── protect/               # ★ Goal-based: net-new Protect Developers content
├── scan/                  # ★ Goal-based: net-new Scan & Analyze content
├── govern/                # ★ Goal-based: net-new Govern & Manage content
├── reference/             # ★ Goal-based: net-new Reference content
│
...
**Path convention:** New pages use goal/topic-based paths (`concepts/`, `protect/`, etc.). Legacy `vet/`/`cloud/`/`apps/` paths are grandfathered — moving them requires a `redirects` entry in `docs.json`.
...
2. Create the file at a **goal-based path** (e.g., `scan/new-platform-integration.mdx`, not `vet/guides/new-integration.mdx`)
...
6. Update `docs.json` under the correct goal tab and group
...
The entire docs site is organized by **user goal** rather than by tool. Users navigate to what they want to accomplish; the tools appear one level deeper.

When adding new content:
- **Don't** create new tool-specific tabs or top-level groups
- **Do** add pages to the appropriate goal tab using the placement table above
