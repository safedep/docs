...
**pmg:** free community API, no account or API key. Supported managers: `npm`, `pnpm`, `yarn`, `bun`, `npx`, `pnpx`, `pip`, `uv`, `poetry`. After `pmg setup install`, `which npm` resolves to the shim under the XDG data dir (`~/.local/share/safedep/pmg/bin/npm` on Linux; not "pmg"). `~/.pmg/bin` is the pre-XDG legacy path (`internal/shim/shim.go`), not current. Config at `config.yml`. Proxy mode currently covers the JS managers only.
