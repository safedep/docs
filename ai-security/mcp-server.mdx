...
<Note>
With SafeDep Cloud, every package your agents vet through the MCP server is recorded per endpoint under [MCP Advisor](/governance/cloud/endpoint-hub/mcp-advisor) in Endpoint Hub, so you can review what they pulled in and the verdict each one got.
</Note>
