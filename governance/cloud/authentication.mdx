...
The SafeDep API is a [gRPC API with a ConnectRPC facade](/reference/api-introduction), not a REST API, so there are no REST paths to call. Whatever transport you use (gRPC over HTTP/2 or JSON over HTTP/1.1), requests carry the same two headers:
...
SafeDep Cloud enforces fair-usage rate limits at the API gateway, measured **per second**, with no hourly quota:
