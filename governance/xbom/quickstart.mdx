...
The primary output is a standardized CycloneDX JSON file. For a quick human-readable view, xBom also prints a link to an interactive HTML report of the BOM, so you can browse the detected components in a browser.
...
**Purpose:**
`xbom` collects anonymous usage telemetry to show which integrations and use cases are common, which guides what to build next. It collects no personally identifiable information or sensitive data.
