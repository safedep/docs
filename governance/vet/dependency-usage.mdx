...
This page focuses on the dependency-usage workflow. For the underlying [Code Analysis](/governance/vet/code-analysis) feature (the framework, supported languages, and options), see its dedicated page.
...
In the scan results, dependencies that the analysis finds used in your source code are marked with the `used-in-code` tag, so you can prioritize them over dependencies that are only declared in a manifest.
