...
SafeDep now supports **Bitbucket** & `vet` integration called `vet-pipe`, is a [Bitbucket Pipe](https://bitbucket.org/product/features/pipelines/integrations) which can easly integrated into any bitbucket CI/CD workflow.
...
To specify this file using `POLICY` variable. 
...
<Note>
When policy violation accur then the Pipeline failes. To overwrite this, set `SKIP_FILTER_CI_FAIL: "true"` in **variables**, to skip **fail** when policy violation happens.
</Note>
