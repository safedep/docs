...
<Info>
  `vet ai discover` is an alias for `vet endpoint scan --kind ai-tool`. When SafeDep credentials are configured, discovered items are automatically synced to SafeDep Cloud's Endpoint Hub — including on upgrade if credentials were already set. See [Inventory Sync](/cloud/endpoint-hub/inventory-sync).
</Info>

<CardGroup cols={2}>
  <Card title="Inventory Sync" icon="cloud" href="/cloud/endpoint-hub/inventory-sync">
    Sync discovered AI tools to SafeDep Cloud's Endpoint Hub
  </Card>
