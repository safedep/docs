...
<Steps>
  ...
  <Step title="Verify Blocking via JFrog">
    ...
    `jf npm install` triggers Xray to index `<Malicious Ingested package>`. If SafeDep has flagged that package as malicious, Xray raises a policy violation and blocks the download.
...
Use `--dry-run` to test the feed without a JFrog client or connection.
