...
<Warning>
  The SafeDep JFrog Xray integration is available with the Threat Intel Feed add-on. To enable it, see [the pricing page](https://safedep.io/pricing/#threat-intel).
</Warning>
...
<Steps>
  ...
  <Step title="Start the integration daemon">
    ...
    On the first run, the daemon starts from now. It does not pull old reports. To seed history
    on the first run, pass `--backfill` with a duration. For example, `--backfill 168h` seeds the
    last 7 days. After the first run, the daemon resumes on its own and ignores `--backfill`.
