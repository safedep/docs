...
1. Download `lib_macos.sh` and `pmg_setup_install_macos.sh` from [`scripts/mdm/macos/`](https://github.com/safedep/pmg/tree/main/scripts/mdm/macos). You can optionally add a sibling `config.yml` for a machine-wide config. If you do, set `cloud.enabled: true` in it. A global config prevents the installer from changing that flag.
