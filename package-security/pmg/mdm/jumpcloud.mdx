...
Create a second Command to catch up users who sign in after the install, without reinstalling PMG. Set it up the same way as install (same shared lib and script, **Run As** `root`), and use this Command body instead:
...
Assign it to the same device group, on a recurring schedule.
