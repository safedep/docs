...
<Steps>
  ...
  <Step title="Attach the scripts">
    Upload `lib_macos.sh` and `pmg_setup_install_macos.sh` from [`scripts/mdm/`](https://github.com/safedep/pmg/tree/main/scripts/mdm). Optionally upload `config.yml` for a machine-wide config. Set the **File Destination** to `/tmp/pmg-mdm/` for each file.
  </Step>
  ...
  <Step title="Add Cloud variables (optional)">
    To sync installs to [Package Guard](/governance/cloud/endpoint-hub/package-guard), define the Command Variables `safedep_api_key` and `safedep_tenant_id`. Mark the API key as a Secret variable.
  </Step>
