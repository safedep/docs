...
MDM deploys enable [SafeDep Cloud](/governance/cloud/quickstart) sync. With cloud credentials set, install runs `pmg cloud sync` for every target user, and installs show up in [Package Guard](/governance/cloud/endpoint-hub/package-guard). You need an API key and Tenant ID from [app.safedep.io/settings/api-keys](https://app.safedep.io/settings/api-keys).

Credentials are stored per user, in the active session's Keychain (macOS) or keyring (Linux), so a user who signs in after the install misses that sync. Pass `--cloud-sync-only` to the install script to sync every target user again, without reinstalling PMG.
