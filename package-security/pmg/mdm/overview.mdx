...
MDM deploys enable [SafeDep Cloud](/governance/cloud/quickstart) sync. Package events already synced automatically once credentials were configured, but the endpoint itself only appeared in [Endpoint Hub](/governance/cloud/endpoint-hub/overview) once something triggered that first sync. Install now runs `pmg cloud sync` for every target user right after setup, so the machine shows up on the dashboard as soon as PMG is deployed.

You need an API key and Tenant ID from [app.safedep.io/settings/api-keys](https://app.safedep.io/settings/api-keys). Credentials are stored per user, in the active session's Keychain (macOS) or keyring (Linux), so a user who signs in after the initial deploy misses that sync. Pass `--cloud-sync-only` to the install script to sync every target user again, without reinstalling PMG.
