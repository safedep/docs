...
3. Assign the policy to a device group. Use a recurring install frequency if new users can be added later. Each run configures the target users and runs `pmg cloud sync` for each one.
