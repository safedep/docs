...
PMG runs on macOS, Linux, and Windows. Native Windows interception uses `.cmd` PATH shims in PowerShell and `cmd.exe`. Windows has no sandbox or background proxy daemon. See the [Windows setup guidance](/package-security/pmg/quickstart#windows).
...
<CardGroup cols={2}>
  ...
  <Card title="Deploy on Kubernetes" icon="cubes" href="/package-security/pmg/kubernetes">
    Protect package installs in Kubernetes workloads and group events by workload.
  </Card>
  <Card title="Deploy with MDM" icon="laptop" href="/package-security/pmg/mdm/overview">
    Install PMG on macOS, Linux, and Windows fleets through your MDM.
  </Card>
