...
1. Download and extract `pmg_Windows_x86_64.zip` from [v0.29.0-edge.3](https://github.com/safedep/pmg/releases/tag/v0.29.0-edge.3). This prerelease includes Windows system install; the latest stable v0.28.1 does not.
...
`npm` should resolve under `%ProgramFiles%\safedep\pmg\bin`, and PMG must block the benign test package. System install puts shims ahead of machine-wide Node.js installations; a per-user install cannot do that.

System install sets Administrators as owner, gives SYSTEM and Administrators full control, and gives Users read and execute access. It disables permission inheritance on the objects it owns. Do not add a user-writable shim directory to the machine PATH.

For managed config, run system install first, then edit the file it creates as administrator. An existing config with different permissions can stop setup. Inspect the reported file and follow the [system-install reference](https://github.com/safedep/pmg/blob/main/docs/system-install.md#configuration). `pmg setup doctor` checks permissions; rerunning setup restores the expected permissions on installed objects.
...
This removes machine shims and their PATH entry. It leaves the binary, its PATH entry, and per-user data. For complete fleet removal, use the [MDM uninstaller](/package-security/pmg/mdm/overview#windows-deployment-checks).
