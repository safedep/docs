...
1. Download and extract `pmg_Windows_x86_64.zip` from the [latest release](https://github.com/safedep/pmg/releases/latest). Windows system install requires PMG v0.29.0 or later.
...
<Warning>
  ...
  Run `pmg config get <key>` after every edit. If the file no longer parses, PMG treats it as locked and falls back to built-in defaults: package blocking still works, but your settings stop applying, and no error appears on the endpoint.
</Warning>
