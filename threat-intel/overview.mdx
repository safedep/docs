...
The SafeDep Threat Intel Feed is a stream of malicious open-source packages. SafeDep builds the feed from its malware analysis and serves it over an API. Each report names one malicious package. A report gives a verdict, its indicators, and the campaign it belongs to. You pull the feed once, then keep a local copy in sync with a cursor.
...
<CardGroup cols={2}>
  ...
  <Card title="Campaigns" icon="diagram-project" href="/threat-intel/campaigns">
    Named groups of related reports that share an actor, technique, or coordinated intent.
  </Card>
</CardGroup>
...
The feed is built for incremental pulls. A report comes back only when it changes, for example a new verdict, new indicators, or a withdrawal. You keep a local mirror, then pull only what changed since your last run.

## Where to go next

<CardGroup cols={2}>
  <Card title="Quickstart" icon="rocket" href="/threat-intel/quickstart">
    Set your credentials and run your first query in a few minutes.
  </Card>
