...
A **package report** is one malicious package with its verdict, affected versions, indicators, and campaign links. See the full shape in the [schema reference](/threat-intel/schema#packagereport).
...
| `filters.ecosystem` | enum | One [`Ecosystem`](/threat-intel/schema#ecosystem) value. |
...
**Response:** `packageReports` (an array of [`PackageReport`](/threat-intel/schema#packagereport)) and `pagination`.
...
**Request:** `reportId` (string). **Response:** `packageReport` (a [`PackageReport`](/threat-intel/schema#packagereport)). An unknown id returns `not_found`.
