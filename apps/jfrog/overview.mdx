...
<Steps>
  ...
  <Step title="Verify Blocking">
    ...
    ```bash
    jf npm install pkg@x.y.z
    ```

    `jf npm install` triggers Xray to index `pkg@x.y.z`. If SafeDep has flagged that package as malicious, Xray raises a policy violation and blocks the download.
  </Step>
</Steps>
...
SafeDep resolves credentials in this order:
